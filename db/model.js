const db = require('../db/mysql_connection.js');
module.exports = class Model {
    ...
    find(id) {
        ...
        let cThis = this;
        return new Promise(function (myResolve, myReject) {
            db.query('SELECT * FROM ?? WHERE idemployee = ?', [cThis.table, id], function (error, result) {
                if (error) throw error;
                myResolve(result[0]);
            })
        });
    ...
    update(id, data) {
        ...
        let cThis = this;
        return new Promise(function (myResolve, myReject) {
            db.query('UPDATE  ?? SET ? WHERE idemployee = ?', [cThis.table, data, id], function (error, result) {
                if (error) throw error;
    ...
    delete(id) {
        ...
        let cThis = this;
        return new Promise(function (myResolve, myReject) {
            db.query('DELETE FROM  ??  WHERE idemployee = ?', [cThis.table, id], function (error, result) {
                if (error) throw error;
                myResolve(result)
